...
Твоя задача - точно и детально извлечь информацию о производственных требованиях сцены.
...
1. ЛОКАЦИЯ (location):
   ✅ "ИНТ. КВАРТИРА - ДЕНЬ" → location = "квартира"
   ✅ "ЭКСТ. УЛИЦА - НОЧЬ" → location = "улица"
   ✅ "В бассейне плавают дети" → location = "бассейн"
   ❌ location = "отделаются дети" (это действие!)
   ❌ location = "плавают" (это глагол!)

2. ПЕРСОНАЖИ (characters):
   ✅ "Иван говорит с Марией" → ["Иван", "Мария"]
   ✅ "Добровольская звонит Пановой" → ["Добровольская", "Панова"]
   ❌ ["толпа", "массовка", "прохожие"] (это массовка!)
   ❌ ["плавают", "идут"] (это глаголы!)

3. РЕКВИЗИТ (props):
   ✅ ДЕТАЛЬНО: ["телефон Nokia 3310", "бутылка плодового вина", "обручальное кольцо", "пустые бутылки (5 шт)", "блокнот в клетку"]
   ✅ С СОСТОЯНИЕМ: ["мокрая одежда", "разбитый телефон", "грязная посуда"]
   ❌ СЛИШКОМ ОБЩЕЕ: ["вещи", "предметы"] (нужны детали!)

4. МАССОВКА (crowd, crowd_count):
   ✅ "толпа из 20 человек" → crowd = "прохожие", crowd_count = 20
   ✅ "массовка: 15 студентов" → crowd = "студенты", crowd_count = 15
   ✅ "курсанты военные (20)" → crowd = "курсанты военные", crowd_count = 20
   ❌ crowd_count = "20 человек" (только число!)

5. КОСТЮМЫ (costumes):
   ✅ ДЕТАЛЬНО: ["деловой костюм серый", "вечернее платье красное", "форма полицейского", "мокрая одежда", "грязная рубашка"]
   ✅ С СОСТОЯНИЕМ: ["костюм в крови", "порванная куртка"]
   ❌ ["одежда", "стандартная одежда"] (слишком общее!)

6. ГРИМ (makeup):
   ✅ ДЕТАЛЬНО: ["разбитая губа", "синяк под глазом", "кровь на лице", "старение +20 лет", "шрам на щеке"]
   ✅ ["бледность", "усталый вид", "грязь на лице"]
   ❌ ["макияж"] (нужны детали!)

7. ТРАНСПОРТ (vehicles):
   ✅ ДЕТАЛЬНО: ["бобик - советский джип", "412 красный москвич", "черная Волга ГАЗ-24"]
   ✅ ["велосипед", "мотоцикл Урал"]
   ❌ ["машина"] (нужна модель/цвет!)

8. СПЕЦЭФФЕКТЫ (vfx):
   ✅ ["туман", "дым", "искры от сварки", "разбитое стекло"]
   ✅ ["кровь (имитация)", "взрыв (пиротехника)"]

9. ТЕХНИЧЕСКИЕ ТРЕБОВАНИЯ:
   - stunts: true если есть драки, падения, трюки
   - pyrotechnics: true если есть взрывы, огонь, дым
   - special_equipment: только съемочное оборудование (кран, дрон, стабилизатор)

Отвечай ТОЛЬКО в формате JSON:
{json.dumps(json_schema, ensure_ascii=False, indent=2)}

ПРАВИЛА:
- Извлекай ВСЕ детали из текста (цвет, размер, состояние, количество)
- Для реквизита указывай конкретные предметы, не обобщай
- Для костюмов и грима указывай состояние (мокрый, грязный, в крови)
- Для транспорта указывай модель и цвет если упомянуты
- Для массовки ВСЕГДА извлекай точное число
- Если информация не найдена: пустая строка "", пустой список [], false, 0
- НЕ придумывай информацию, которой нет в тексте
- JSON должен быть валидным"""

        user_prompt = f"""Извлеки ДЕТАЛЬНУЮ информацию из сцены:

СЦЕНА №{scene_number}

{scene_text}

Верни JSON с МАКСИМАЛЬНО ДЕТАЛЬНЫМИ данными:"""
    ...
    def extract_entities_batch(
        ...
        for i in range(0, len(scenes), batch_size):
            ...
            # КРИТИЧЕСКАЯ ОЧИСТКА ПАМЯТИ после каждого батча
            del prompts
            del responses
            if self.device == 'cuda':
                torch.cuda.empty_cache()
                ...
                mem_allocated = torch.cuda.memory_allocated() / 1024**3
                mem_reserved = torch.cuda.memory_reserved() / 1024**3
                logger.info(f"После батча: {mem_allocated:.2f}GB выделено, {mem_reserved:.2f}GB зарезервировано")
            gc.collect()
