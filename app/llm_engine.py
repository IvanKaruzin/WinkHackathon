...
Верни JSON-массив сценариев:"""
    ...
    def _generate_entity_extraction_prompt(self, scene_text: str, scene_number: str, entities_to_extract: List[str]) -> str:
        ...
        # Упрощенный промпт специально для Qwen2.5
        system_prompt = """Ты - ассистент для анализа киносценариев. Извлекай информацию из сцен и возвращай ТОЛЬКО валидный JSON-объект."""

        user_prompt = f"""Проанализируй сцену и верни JSON-объект с данными. Пиши на русском.
...
ВАЖНО:
- location: название места (квартира, улица, офис)
- time_of_day: ДЕНЬ, НОЧЬ, УТРО или ВЕЧЕР
- scene_type: ИНТ или ЭКСТ
- characters: список имен персонажей
- synopsis: краткое описание действия
...
Верни ТОЛЬКО JSON-объект, без дополнительного текста:"""
    ...
    def _extract_json(self, text: str) -> Optional[Any]:
        ...
        # Находим первую открывающую ФИГУРНУЮ скобку (только объекты, не массивы)
        start_idx = -1
        for i, char in enumerate(text):
            if char == '{':  # Ищем только объекты
                start_idx = i
                break
        
        if start_idx == -1:
            # Если не нашли объект, попробуем найти массив
            for i, char in enumerate(text):
                if char == '[':
                    start_idx = i
                    break
        ...
        # Если нашли массив, логируем предупреждение
        if bracket == '[':
            logger.warning(f"Модель вернула массив вместо объекта. Текст: {text[:200]}")
    ...
    def extract_entities_batch(
        ...
        for i in range(0, len(scenes), batch_size):
            ...
            for scene, response in zip(batch, responses):
                ...
                logger.debug(f"Сцена {scene.scene_number} - ответ модели: {response[:200]}...")
                ...
                if not isinstance(entities, dict):
                    if isinstance(entities, list):
                        logger.warning(f"Сцена {scene.scene_number}: получен список вместо словаря. Ответ: {response[:300]}")
                    else:
                        logger.warning(f"Сцена {scene.scene_number}: неожиданный тип {type(entities)}. Ответ: {response[:300]}")
                    entities = {}
